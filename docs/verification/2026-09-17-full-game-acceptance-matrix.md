...
| C2 | 13 building types: costs, prereqs, queue stacking, server timers, offline catch-up | IMPLEMENTED + TESTED (current, §1); **VERIFIED LIVE (current tip, 2026-09-20, `21b917b`) for cost, server timer, exact-time completion, idempotent replay, stale-version conflict and ownership** — re-drive steps 5–8, 14: `village.build.queue` timber deducted exactly {75, 90, 60}; timer 720 s = `buildingDurationSeconds`; replay byte-identical with no duplicate row; stale `expectedWorldVersion` → 409 `WORLD_VERSION_CONFLICT`; other player → 409 `FORBIDDEN` (both for a foreign village and a foreign `commandId`); **level 1 → 2 landed with its notification stamped exactly at `completesAt`** and production moved 28 → 33/h. Offline catch-up shown live for a *recruit* job (step 11: militia completed at its `completesAt` while the server was down); queue stacking shown live in B10a (quarry `waiting`). **Prerequisite refusals and the other 11 building types NOT exercised live** (unit-tested) | — (done; Cursor ran it) / Claude reviews | Evidence note steps 5–8, 11, 14; build-queue tests; audit §6.1 (historical) | One building live at this tip, as in the 2026-08-29 exercise. |
...
**Artifact counts (revision 2, so the summary matches the tables):** §2 holds **63 feature rows** (A 4 · B 11 incl. B10a/B10b · C 9 · D 4 · E 12 · F 9 · G 5 · H 9); §3 holds **12 practice rows** (S-01…S-10, G-01, G-02); §4 holds **10 decision lines**, of which 1 is RESOLVED and 1 is OPEN design review, the rest DECISION-HELD. Rows carrying a **current-tip live PASS: 8** (S-10 on `0b7a34a`; B10a on `38610e0`, revision 3; **A1, B1, B7, C1, C2 and the server half of A2 on `21b917b`, revision 4** — C1 minus the warehouse-cap branch, C2 for one building type, as their rows say). Rows carrying a **VERIFIED LIVE that is historical only: 5** (A2's Roblox-side half, B6, E10, F6, G1) — none of them certifies `9b478db`; all five need the Studio prerequisite. Everything else Studio- or device-dependent is NOT RUN.
