...
**Artifact counts (revision 2, so the summary matches the tables):** §2 holds **63 feature rows** (A 4 · B 11 incl. B10a/B10b · C 9 · D 4 · E 12 · F 9 · G 5 · H 9); §3 holds **12 practice rows** (S-01…S-10, G-01, G-02); §4 holds **10 decision lines**, of which 1 is RESOLVED and 1 is OPEN design review, the rest DECISION-HELD. Rows carrying a **current-tip live PASS: 11** (S-10 on `0b7a34a`; B10a on `38610e0`, revision 3; **A1, B1, B7, C1, C2 and the server half of A2 on `21b917b`, revision 4** — C1 minus the warehouse-cap branch, C2 for one completed building type, as their rows say; **B8 on `69b7548`, revision 5** — kill -9 only, power loss not simulated; **H1 and H2 on `03a9f41`, revision 6** — H1 at the coded defaults on loopback, not behind a proxy and not tuned). Rows carrying a **VERIFIED LIVE that is historical only: 5** (A2's Roblox-side half, B6, E10, F6, G1) — none of them certifies `9b478db`; all five need the Studio prerequisite. Everything else Studio- or device-dependent is NOT RUN.
...
**HTTP-only claims still resting on tests alone after revision 6** (for whoever picks the next slice): C1's warehouse-cap branch (hours of accrual at 28/h, or a dev seed level); C2's other prerequisite rules and completion of any building other than timber (720 s+ each); D1–D3 fog/scouting and E1 march launch over HTTP (need scouts and a march in flight — the same prerequisite as the B8 battle-settle variant above, so they are best done together); C6's over-cap recruit (a known-BROKEN row — a drill would document the bug, not pass); H1 behind a reverse proxy (H6).
